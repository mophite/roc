...
type Strategy interface {
	...
	StraightByAddr(scope, address string) (next *conn.Conn, err error)

	//StraightById direct call
	StraightById(scope, id string) (next *conn.Conn, err error)
...
func (s *strategy) StraightByAddr(scope, address string) (*conn.Conn, error) {
	p, err := s.getOrSet(scope)
	if err != nil {
		return nil, err
	}
...
func (s *strategy) StraightById(scope, id string) (*conn.Conn, error) {
	p, err := s.getOrSet(scope)
	if err != nil {
		return nil, err
	}
